...
\begin{abstract}
Conventional practice for hard LLM tasks is to scale up the number of agents and aggregate their answers~\citep{li2024moreagents}. We show this is the wrong default on knowledge-bound tasks: scaling from one to four GPT-4o agents drops accuracy by $32.6$pp on legal classification while medical QA saturates near $N{=}1$; open-context tasks usually improve at $N{=}2$ but never monotonically. We explain when ensembling helps with an exact identity $\Delta = (1{-}\alpha)\,r - \alpha\,(1{-}p)$ --- $\alpha$ the one-agent baseline, $p$ the preservation rate, $r$ the recovery rate --- reducing ``does multi-agent help?'' to a verifiable boundary $r > \alpha(1{-}p)/(1{-}\alpha)$ estimable from a small paired pilot at $N{\in}\{1,2\}$. A single LLM-judged feature, \emph{knowledge concentration} $K$, scoreable from task text in minutes, predicts $\alpha$ ex ante; the rule ``use one agent if $K \geq 0.6$, else two'' matches $7$ of $8$ main-benchmark cells under leave-one-domain-out CV, tying an oracle $\alpha$-rule that itself requires a single-agent eval. An out-of-distribution extension to math and multi-step reasoning exposes the limit of any single text-only feature: math has high $\alpha$ despite low $K$. Practitioners should score $K$ first and run the paired pilot when feasible; on narrow-expert tasks, adding agents costs both accuracy and ${\sim}N{\times}$ tokens.
\end{abstract}
