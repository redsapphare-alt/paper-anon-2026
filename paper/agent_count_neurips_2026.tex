...
\begin{abstract}
Conventional practice for hard LLM tasks is to scale up the number of agents and aggregate their answers~\citep{li2024moreagents}. We show this is the wrong default on knowledge-bound tasks: across legal classification, medical question answering, formal logic, and long-context reading, scaling from one to several agents either erodes accuracy by tens of percentage points, saturates immediately, or peaks short of the maximum agent count --- never delivering the monotone gains the prevailing recipe assumes. We explain when ensembling helps with an exact algebraic identity that decomposes the one-to-many accuracy change into a recovery term (the rate at which a single agent's wrong answer is overturned by the vote) and a preservation term (the rate at which its correct answer survives). This reduces ``does multi-agent help?'' to a verifiable inequality estimable from a small paired pilot. A single LLM-judged feature, \emph{knowledge concentration}, scoreable from task text alone in minutes, predicts the one-agent baseline ex ante; a simple threshold rule on it (``use one agent when knowledge is concentrated, else two'') matches nearly all of our main-benchmark cells under leave-one-domain-out cross-validation, tying an oracle rule that itself requires running a single-agent evaluation first. An out-of-distribution extension to math and multi-step reasoning exposes the limit of any single text-only feature: math benefits from voting despite looking knowledge-light, so practitioners should pair the feature score with the paired pilot when feasible. On narrow-expert tasks, adding agents costs both accuracy and linearly more compute.
\end{abstract}
...
Multi-agent systems built on LLMs are now a default architectural pattern. \citet{li2024moreagents} argue that simply increasing the number of agents and voting over outputs is enough --- ensemble accuracy scales monotonically with $N$ --- and a growing body of work assumes the same monotonicity across domains.

We show this view is incomplete (Figure~\ref{fig:teaser}). On four real-world NLP benchmarks (\S\ref{sec:experiments}) with two strong base models, multi-agent scaling produces \emph{four qualitatively distinct} accuracy curves (Figure~\ref{fig:curves}): monotone decreasing on legal, near-flat saturation on medical, inverted-U on logic/Claude and long-context, and non-monotone on logic/GPT-4o. On LegalBench, GPT-4o drops $56.4 \to 23.8\%$ as $N{=}1{\to}4$ --- a $32.6$pp regression robust under bootstrap and consistent across both base models. ``More agents'' is not just unhelpful here: it is actively harmful.

We answer two questions: (a)~\emph{why} does the same protocol produce opposite-sign effects, by an exact algebraic decomposition (\S\ref{sec:mechanism}); and (b)~given a new task, can we decide ex-ante whether to deploy one strong agent or many, by a single-feature predictor (\S\ref{sec:phase3}).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig0_teaser_drawio.pdf}
\caption{\textbf{Overview of our argument.} (\textbf{Top}) The conventional ``more agents is all you need'' recipe~\citep{li2024moreagents} predicts monotone gains with $N$; on knowledge-bound NLP we find the opposite (legal/GPT-4o $-32.6$pp at $N{=}4$). (\textbf{Middle}) Six selected (domain $\times$ model) cells under one fixed sampling-and-vote protocol: the five \emph{main} cells shown (legal/GPT-4o, legal/Claude, medical/GPT-4o, logic/GPT-4o, long-context/GPT-4o) plus one \emph{tier-2} OOD cell (reasoning/GPT-4o) span monotone-decreasing, flat, inverted-U, non-monotone, and saturated shapes; the largest positive $\Delta$ in our data is $+20.3$pp on logic/GPT-4o at $N{=}4$ (main benchmark) and $+9.1$pp on reasoning/GPT-4o at $N{=}2$ (tier-2 OOD). (\textbf{Bottom}) Each task is placed on the knowledge-concentration $K$ axis (squares mark tier-2). The threshold rule $N^{*}{=}1$ if $K{\geq}0.6$ else $N^{*}{=}2$ matches $7/8$ main-benchmark cells under leave-one-domain-out CV ($87.5\%$); the boundary inequality $r > \alpha(1{-}p)/(1{-}\alpha)$ (\S\ref{sec:mechanism}) gives an exact help-or-hurt sign from a small paired pilot.}
\label{fig:teaser}
\end{figure}

Our contributions are:
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Empirical falsification of ``more is better''} (\S\ref{sec:results}): legal/GPT-4o $-32.6$pp ($N{=}1{\to}4$, McNemar $p{<}10^{-100}$); inverted-U or non-monotone curves on logic and long-context (logic/GPT-4o $\Delta_{4-1}{=}{+}20.3$pp, the largest positive $\Delta$ in our data); a tier-2 OOD extension to math/reasoning showing the same algebraic regime ($+9.1$pp on reasoning/GPT-4o at $N{=}2$, large negative $\Delta$ on math).
...
For each (domain, model, $N$) cell with $N{\in}\{1,2,3,4\}$ we run a fixed centralised orchestration: $N$ agents receive identical prompts at temperature $1.0$ (matching~\citet{li2024moreagents}'s primary setting), a domain-aware extractor (\texttt{extractor\_v2.py}; boxed/``Final Answer''/last-bold priority) parses each response, and the orchestrator takes a majority vote with ties going to the lowest-indexed agent. Base models are \texttt{gpt-4o-2024-11-20} (OpenAI) and \texttt{claude-sonnet-4-6} (Anthropic), accessed via their respective production APIs; main and tier-2 domains together contribute ${\approx}103{,}000$ majority-voted question outcomes across $48$ planned cells. We audited every result file (\texttt{contamination\_audit\_v2.py} on main, \texttt{validate\_v2\_fixed.py} on tier-2; Appendix~\ref{app:protocol-detail}) and retain $\mathbf{12}$ trustable (domain $\times$ model) curves --- $8$ main, $4$ tier-2 --- after re-running two cells with sample-size issues (medical/Claude/1-agent and logic/GPT-4o/4-agent). Two original issues were caught and corrected: (a) an empty-prediction matching bug inflating accuracy by up to $+28$pp on code (whole code domain withdrawn); (b) a storage-time truncation in an earlier tier-2 collection ($200$/$2000$-character caps that scaled with $N$), re-run with full-response logging and per-agent traces. \label{sec:contamination}


\section{Empirical Observations}
\label{sec:results}

Figure~\ref{fig:curves} plots accuracy versus agent count for the four trustable domains and two base models. Four qualitatively distinct curve shapes are visible.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig1_curves_by_domain.pdf}
\caption{Accuracy as a function of agent count, by task domain and base model (main 4 trustable domains; code/HumanEval excluded after the empty-prediction matching bug, \S\ref{sec:contamination}). Shaded bands are bootstrap $95\%$ CIs. Both base models exhibit monotone decreasing curves on legal, an inverted-U at $N{=}2$ on long-context (and Claude/logic), a non-monotone curve on logic/GPT-4o (peak $N{=}4$), and a flat curve on medical. Domain titles include $K$ (knowledge concentration, see \S\ref{sec:features}).}
\label{fig:curves}
\end{figure}

\paragraph{Monotone-decreasing on legal; near-flat on medical.} On LegalBench both base models lose accuracy as $N$ grows: GPT-4o drops $56.4 \to 44.0 \to 34.2 \to 23.8$ ($\Delta_{4-1}{=}{-}32.6$pp, $95\%$ bootstrap CI $[-34.6,-30.6]$); Claude drops $87.8 \to 77.7 \to 72.4 \to 72.0$ ($\Delta_{4-1}{=}{-}15.8$pp, $[-17.3,-14.4]$). McNemar paired tests on shared question identifiers reject equal accuracy at $p < 10^{-100}$ for both models. On medical QA, both base models are approximately flat (GPT-4o $28.5 \to 25.8$, peak $N{=}1$; Claude $24.6 \to 19.0 \to 22.9 \to 25.0$ with peak at $N{=}4$ by $0.5$pp over $N{=}1$, statistically indistinguishable from saturation).

\paragraph{Inverted-U on long-context (and Claude/logic); non-monotone on logic/GPT-4o.} On long-context QuALITY both models peak at $N{=}2$ with sharper concavity for Claude ($3.3 \to 19.7 \to 7.4 \to 1.6$). On MMLU logic, Claude shows a cleaner inverted-U ($18.0 \to 22.3 \to 20.3 \to 19.7$, peak $N{=}2$) while GPT-4o exhibits a non-monotone curve ($21.4 \to 38.6 \to 24.8 \to 41.7$) with the largest positive $\Delta$ in our data ($\Delta_{4-1}{=}{+}20.3$pp) and a strong $N{=}2$ peak ($\Delta_{2-1}{=}{+}17.2$pp).

\paragraph{Tier-2 extension (math, reasoning).} math/Claude ($\alpha{=}0.94$) and math/GPT-4o ($\alpha{=}0.89$) both peak at $N{=}1$ with sharp dips at $N{=}2$ ($-15.0$pp, $-11.8$pp) and partial recovery at $N{=}3,4$. reasoning/GPT-4o peaks at $N{=}2$ ($\Delta_{2-1}{=}{+}9.1$pp, the largest positive $\Delta$ in tier-2; the overall largest is logic/GPT-4o at $\Delta_{4-1}{=}{+}20.3$pp). reasoning/Claude saturates near $\alpha{=}0.92$ across all $N$ (variance $<\!1$pp) and we treat it as a saturation case rather than a meaningful peak. The full per-cell $\alpha, p, r$ table is in Appendix~\ref{app:in-progress}.

\paragraph{Across both models the same domain-level pattern dominates.} Of $8$ main-benchmark cells, the within-domain agreement is consistent: legal both decline, medical both flat-or-near-flat, long-context both peak at $N{=}2$. The one within-domain cross-model difference is logic, where GPT-4o is non-monotone while Claude is cleanly inverted-U at $N{=}2$. On tier-2 (math, reasoning) the agreement is $4/4$ at the within-domain level (math peaks at $N{=}1$ for both base models; reasoning peaks at $N{=}2$ for GPT-4o while Claude is saturated). This cross-model robustness rules out a model-specific explanation and motivates a task-feature account.
...
\begin{equation}
  ...
  \label{eq:boundary}
\end{equation}
The intuition: vote-based aggregation must \emph{recover} more wrong-answer mass than it \emph{loses} from correct-answer mass, weighted by the prior (in)correctness rate. Verification of identity~\eqref{eq:decomp} on all $32$ trustable (domain, model, $N{\geq}2$) cells (main: $20$ paired cells across $4$ domains $\times$ $2$ models; tier-2: $12$ paired cells across math, reasoning $\times$ $2$ models) gives a maximum residual of $1.0\times10^{-15}$ (Table~\ref{tab:asymmetry}; \texttt{analysis/mechanism\_voting\_asymmetry.csv} and \texttt{analysis/tier2\_v2\_decomposition.csv}). The decomposition holds with no model assumption, so this is a sanity check on our paired bookkeeping rather than a hypothesis test --- but it confirms the absence of any item-pairing or extraction-mismatch issues that would bias downstream analysis.

\paragraph{Three empirical findings.} (i)~$p > r$ in $31/32$ cells (mean gap $+0.40$, Figure~\ref{fig:mechanism}a; the lone violator is long-context/GPT-4o at $N{=}3$ with $p, r$ both within $0.003$ of zero --- a numerical edge case). This follows from item-difficulty heterogeneity (easy items right at every $N$; hard items wrong) rather than a separate property of voting (Appendix~\ref{app:mechanism-null}). (ii)~Boundary~\eqref{eq:boundary} is consistent with $\mathrm{sign}(\Delta)$ on every observed cell (Figure~\ref{fig:mechanism}c); we emphasise this is a sanity check rather than a predictive test, since the boundary uses the same $(\alpha, p, r)$ that determine $\Delta$ algebraically. The diagnostic value is in projecting the cell onto the $(r, \alpha(1{-}p)/(1{-}\alpha))$ plane to read off the help/hurt margin; reasoning/Claude saturates at $\alpha{\approx}0.92$ and the boundary correctly tags $\Delta{\approx}0$. (iii)~On the main benchmark $K$ acts on the boundary through $\alpha$ alone: $r(K, \alpha){=}{+}0.77$ ($p{=}0.044$ uncorrected; multiple-comparison-aware Bonferroni-adjusted $p{\approx}0.13$); $r(K, r_{\mathrm{recover}}){=}{+}0.37$ and $r(K, 1{-}p){=}{-}0.12$ are null. Tier-2 cells decouple $K{\leftrightarrow}\alpha$ (math: high $\alpha$, moderate $K$), confining the channel claim to the main four (\S\ref{sec:phase3}).
...
\paragraph{Paired-test confirmation.} The Friedman omnibus rejects equality of $N{\in}\{1,2,3,4\}$ accuracy at $p{<}0.001$ on every cell with full data. Pairwise McNemar tests~\citep{mcnemar1947} find significant decreases on the high-$K$ tasks (legal/GPT $\Delta_{4-1}{=}{-}32.6$pp, $p{<}10^{-100}$, Cohen's $h{=}{-}0.68$~\citep{cohen1988}; legal/Claude $-15.8$pp, $p{<}10^{-100}$; medical/GPT $-2.7$pp, $p{=}0.0006$) and non-significant changes on low-$K$ inverted-U cells (Figure~\ref{fig:paired-tests}; full tables in Appendix~\ref{app:per-cell}). The largest negative deltas survive any reasonable Bonferroni / Benjamini-Hochberg correction over $24$ paired comparisons ($8$ cells $\times$ $3$ $N$ values); $p{<}10^{-100}$ leaves an enormous margin even after $24{\times}$ adjustment. Bootstrap CIs use $2{,}000$ resamples~\citep{efron1993bootstrap}.

\paragraph{Out-of-distribution validation: math and reasoning.} On $4$ tier-2 cells (gsm8k, ARC-Challenge $\times$ GPT-4o, Claude~Sonnet~4.6) the decomposition~\eqref{eq:decomp} reproduces every $\Delta$ to numerical precision (residual $|{<}10^{-15}|$, $12$ new cells); reasoning/GPT-4o at $N{=}2$ ($\alpha{=}0.84$, $p{=}0.97$, $r{=}0.73$, $\Delta{=}{+}9.1$pp) satisfies the boundary by $4.8{\times}$. The $K$-$\alpha$ confound becomes empirically separable: math has rubric-anchored $K{\approx}0.30$ yet very high $\alpha$ ($0.94$ Claude, $0.89$ GPT-4o), so the K-rule predicts $N^{*}{=}2$ and misses while the $\alpha$-rule hits; reasoning inverts the relationship. Both rules score $9/12$ on the combined benchmark (LOO; Wilson $95\%$ CI $[0.47, 0.91]$) with disjoint failures. A $2$-d $(K, \alpha)$ logistic regression overfits at $n{=}12$. Take-away: when $K$ and $\alpha$ co-move ($r{=}{+}0.77$ on main 7) $K$ matches the oracle $\alpha$-rule for free; when they decouple, the small paired pilot of \S\ref{sec:discussion} is the safety net.
...
\paragraph{Why $K$ dominates.} On our four-domain slice $V$ saturates and $D, S$ have weak inter-judge agreement; $K$ is the only axis with material variance and reproduction across judges, and it is a strong ex-ante predictor of $\alpha$ ($r{=}{+}0.77$). We do not claim $K$ is causally privileged over $\alpha$: practitioners who can measure $\alpha$ should use it. $K$'s value is being judgeable from text in under $15$ minutes, before any agents are run.
...
\paragraph{Limitations.} \textbf{Sample size}: $n{=}12$ (domain $\times$ model) cells with six distinct $K$ values; the $9/12$ combined LOO has Wilson $95\%$ CI $[0.47, 0.91]$ --- a fact we report transparently rather than rounding. \textbf{Identifiability}: $K$ and $\alpha$ co-move on the main $8$ ($r(K,\alpha){=}{+}0.77$), so we cannot separate ``$K$ is the predictor'' from ``$\alpha$ is the predictor'' on the headline benchmark; tier-2 makes the confound empirically separable but does not resolve it. \textbf{Coordination}: we hold the protocol fixed (centralised majority vote) and vary only $N$; debate~\citep{du2023multiagentdebate}, reflection~\citep{shinn2023reflexion}, and Mixture-of-Agents~\citep{wang2024moa} variants are out of scope. \textbf{Stochasticity}: each cell is one run at temperature $1.0$; we report bootstrap CIs over questions but not over runs --- with multi-seed runs the medical/Claude near-flat curve and logic/GPT-4o non-monotone curve might tighten or wash out. \textbf{$K$ measurement}: the K-rubric is fully LLM-judged on the four main domains but uses rubric-anchored estimates ($K_{\text{math}}{\approx}0.30$, $K_{\text{reasoning}}{\approx}0.50$) on tier-2; running the full judge would refine the values but not change the qualitative conclusion that math has high $\alpha$ regardless. \textbf{Item-level signal} ($\rho{=}{-}0.21$) is modest. \textbf{Excluded data}: only the code domain (HumanEval) was permanently dropped after an empty-prediction matching bug; a separate Gemini~2.5~Pro replication uncovered storage-time truncation and prompt-format mismatches and is treated as a data-collection artefact deferred to follow-up.


\section{Conclusion}
\label{sec:conclusion}

Across ${\approx}103{,}000$ majority-voted question outcomes on $4$ main NLP domains plus a tier-2 OOD extension on two base models, multi-agent voting~\citep{li2024moreagents,wang2023selfconsistency} does not scale monotonically with $N$: knowledge-bound tasks decrease (legal/GPT-4o $-32.6$pp, McNemar $p{<}10^{-100}$) and open-context tasks peak at $N{\geq}2$ (logic/GPT-4o $+20.3$pp at $N{=}4$). The exact decomposition $\Delta=(1{-}\alpha)r-\alpha(1{-}p)$ and asymmetry $p \gg r$ (in $31/32$ cells, item-heterogeneity-driven) yield the boundary $r > \alpha(1{-}p)/(1{-}\alpha)$, reproducing every $\Delta$ to numerical precision. The text-judged feature $K$ tracks $\alpha$ ($r{=}{+}0.77$ on main 7) and ``$N^{*}{=}1$ if $K{\geq}0.6$ else $2$'' matches $7/8$ main cells (LOO), tying an oracle $\alpha$-rule. Tier-2 cells decouple $K$ and $\alpha$ (math: high $\alpha$ despite low $K$); both rules score $9/12$ on the combined benchmark with disjoint failures.

\paragraph{Practical takeaway.} On narrow-expert domains, default to one strong agent --- the opposite of the prevailing recipe, saving both accuracy and ${\sim}N{\times}$ compute. On open-context tasks $N{=}2$ is typically optimal; $N{\geq}4$ is rarely justified.

\paragraph{Future work.} (i)~Joint variation of coordination architecture and roles~\citep{google2025scalingagents,du2023multiagentdebate}. (ii)~A continuous predictor on $(\alpha, p, r)$ from a paired pilot. (iii)~Reasoning-specialised models~\citep{deepseek2025r1} where high-$\alpha$ math may pull the boundary toward $N^{*}{=}1$.
...
\begin{table}[h]
\centering
\caption{Trustable (domain $\times$ model) cells used in the headline analysis. Accuracy at each agent count, with observed peak agent count and curve shape. Bootstrap $95\%$ CIs are over $2{,}000$ resamples of the question-level outcomes per cell. medical/Claude/1-agent and logic/GPT-4o/4-agent are re-run from a contaminated/partial earlier collection (full $5{,}000$ and $1{,}500$ items respectively).}
\label{tab:trustable}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{l@{\hskip 8pt}l c c c c c l}
\toprule
...
medical       & Claude~4.6  & $0.246$ & $0.190$ & $0.229$ & $0.250$ & $4$ & near-saturated \\
logic         & GPT-4o      & $0.214$ & $0.386$ & $0.248$ & $0.417$ & $4$ & non-monotone \\
...
\paragraph{Models, dates, infrastructure.} Base models are GPT-4o (OpenAI, 2024) and Claude~Sonnet~4.6 (Anthropic, 2026), accessed via the providers' production APIs between 2026-04-30 and 2026-05-01. Sampling temperature is fixed at $1.0$ for the agents (independent draws) and $0.0$ for the deterministic extractor. Gemini~2.5~Pro runs are partial.
...
\item \textbf{Sample-size drops ($2$ files, since re-run):} an earlier collection of \emph{medical/Claude/1-agent} processed only $150/5{,}000$ items and \emph{logic/GPT-4o/4-agent} only $450/1{,}500$; both were re-run for v3 of this paper with the corrected pipeline (full responses, per-agent traces) and now report $5{,}000$ and $1{,}500$ items respectively (results in \texttt{results/v3/v3\_priority1/}).
\end{itemize}
...
Splitting items into five $K$-quantile bins (a non-empty subset fills four bins given the bimodal $K$ distribution across our four domains) and reporting mean $\mathrm{benefit}_{2{\leftarrow}1}$ per bin: the lowest two quintiles (mean $K \approx 0.12$ and $0.50$) show positive average benefit ($+0.118$, $+0.143$); the highest quintile (mean $K \approx 0.87$) is approximately zero; the second-highest (mean $K \approx 0.80$) shows the strongest negative benefit ($-0.210$). The non-monotonicity in the highest quintile reflects medical/Claude's near-flat curve. The full per-domain scatter is in Figure~\ref{fig:per-question}.
