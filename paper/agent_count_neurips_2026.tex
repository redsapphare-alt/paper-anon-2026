...
\paragraph{Three empirical findings.} (i)~$p > r$ in $31/32$ cells (mean gap $+0.40$, Figure~\ref{fig:mechanism}a; the lone violator is long-context/GPT-4o at $N{=}3$ with $p, r$ both within $0.003$ of zero --- a numerical edge case). This follows from item-difficulty heterogeneity (easy items right at every $N$; hard items wrong) rather than a separate property of voting (Appendix~\ref{app:mechanism-null}). (ii)~Boundary~\eqref{eq:boundary} separates helpful from harmful cells (Figure~\ref{fig:mechanism}c); reasoning/Claude saturates at $\alpha{\approx}0.92$ and the boundary correctly predicts $\Delta{\approx}0$. (iii)~On the main benchmark $K$ acts on the boundary through $\alpha$ alone: $r(K, \alpha){=}{+}0.77$ ($p{=}0.044$); $r(K, r_{\mathrm{recover}}){=}{+}0.37$ and $r(K, 1{-}p){=}{-}0.12$ are null. Tier-2 cells decouple $K{\leftrightarrow}\alpha$ (math: high $\alpha$, moderate $K$), confining the channel claim to the main four (\S\ref{sec:phase3}).
