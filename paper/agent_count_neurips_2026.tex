...
We show this view is incomplete (Figure~\ref{fig:teaser}). On four real-world NLP benchmarks (\S\ref{sec:experiments}) with two strong base models, multi-agent scaling produces qualitatively different accuracy curves: monotonically decreasing on knowledge-bound tasks and inverted-U with peak at $N{=}2$ on open-context tasks (Figure~\ref{fig:curves}). On LegalBench, GPT-4o drops $56.4 \to 23.8\%$ as $N{=}1{\to}4$ --- a $32.6$pp regression robust under bootstrap and consistent across both base models. ``More agents'' is not just unhelpful here: it is actively harmful.
...
\paragraph{Decreasing on knowledge-bound tasks (legal, medical).} On LegalBench both base models lose accuracy as $N$ grows: GPT-4o drops $56.4 \to 44.0 \to 34.2 \to 23.8$ ($\Delta_{4-1}{=}{-}32.6$pp, $95\%$ bootstrap CI $[-34.6,-30.6]$); Claude drops $87.8 \to 77.7 \to 72.4 \to 72.0$ ($\Delta_{4-1}{=}{-}15.8$pp, $[-17.3,-14.4]$). McNemar paired tests on shared question identifiers reject equal accuracy at $p < 10^{-100}$ for both models. On medical QA, GPT-4o is approximately flat ($28.5 \to 25.8$, peak $N{=}1$); Claude is also approximately flat ($25.4 \to 19.0 \to 22.9 \to 25.0$) with peak at $N{=}4$ by $0.4$pp over $N{=}1$, statistically indistinguishable from saturation.

\paragraph{Non-monotone on logic and long-context QA.} On MMLU logic, GPT-4o exhibits a non-monotone curve ($21.4 \to 38.6 \to 24.8 \to 41.7$) with the largest positive $\Delta$ in our data ($\Delta_{4-1}{=}{+}20.3$pp) and a strong $N{=}2$ peak ($\Delta_{2-1}{=}{+}17.2$pp); Claude shows a cleaner inverted-U ($18.0 \to 22.3 \to 20.3 \to 19.7$, peak $N{=}2$). On long-context QuALITY both models peak at $N{=}2$ with sharper concavity for Claude ($3.3 \to 19.7 \to 7.4 \to 1.6$).
